...
def parse_inter_region_connections():
    ...
    try:
        ...
        if header_row is None:
            # 데이터가 바로 시작하는 경우 (첫 번째 행부터)
            for i, row in df.iterrows():
                if pd.notna(row.iloc[0]) and '_' in str(row.iloc[0]):
                    line_data = {
                        'name': str(row.iloc[0]),
                        'bus0': f"{str(row.iloc[1])}_EL" if pd.notna(row.iloc[1]) else '',
                        'bus1': f"{str(row.iloc[2])}_EL" if pd.notna(row.iloc[2]) else '',
                        'carrier': 'AC',
                        'x': float(row.iloc[6]) if len(row) > 6 and pd.notna(row.iloc[6]) else 0.01,
                        'r': float(row.iloc[7]) if len(row) > 7 and pd.notna(row.iloc[7]) else 0.001,
                        's_nom': float(row.iloc[5]) if len(row) > 5 and pd.notna(row.iloc[5]) else 1000,
                        'length': float(row.iloc[4]) if len(row) > 4 and pd.notna(row.iloc[4]) else 100,
                        'v_nom': 345
                    }
...
def parse_constraints():
    ...
    try:
        df = pd.read_excel(INTERFACE_XLSX, sheet_name='constraints')
...
def create_integrated_data():
    ...
    try:
        ...
        # 재생에너지 패턴 (기존 데이터 사용)
        try:
            renewable_patterns_df = pd.read_excel(INTERFACE_XLSX, sheet_name='renewable_patterns')
        except:
            # 기본 패턴 생성
        ...
        # 부하 패턴 (기존 데이터 사용)
        try:
            load_patterns_df = pd.read_excel(INTERFACE_XLSX, sheet_name='load_patterns')
        except:
            # 기본 패턴 생성
...
if __name__ == "__main__":
    print("=== 완전한 integrated_input_data.xlsx 생성 (lines + constraints 포함) ===")
